...
export class Environment {
  ...
  /**
   * Enable generated reports for documents. When no Agent service URL is set,
   * Outline uses a local mock generator so local and Docker installs still work.
   */
  @IsOptional()
  @IsBoolean()
  public AI_REPORTS_ENABLED =
    this.toOptionalBoolean(environment.AI_REPORTS_ENABLED) ?? true;

  /**
   * Optional external Agent service endpoint for document report generation.
   */
  @IsOptional()
  @IsUrl({
    protocols: ["http", "https"],
  ...
  public AGENT_REPORT_SERVICE_URL = this.toOptionalString(
    environment.AGENT_REPORT_SERVICE_URL
  );

  /**
   * Optional bearer token for the external Agent service.
   */
  @IsOptional()
  public AGENT_REPORT_SERVICE_TOKEN = this.toOptionalString(
    environment.AGENT_REPORT_SERVICE_TOKEN
  );

  /**
   * Enable automatic permanent deletion of documents older than the configured
   * retention period. This is disabled by default because deletion is
   * irreversible.
   */
  @IsOptional()
  @IsBoolean()
  public DOCUMENT_RETENTION_ENABLED =
    this.toOptionalBoolean(environment.DOCUMENT_RETENTION_ENABLED) ?? false;

  /**
   * Number of days to retain documents before automatic permanent deletion.
   */
  @IsOptional()
  @IsNumber()
  @Min(1)
  public DOCUMENT_RETENTION_DAYS =
    this.toOptionalNumber(environment.DOCUMENT_RETENTION_DAYS) ?? 30;

  /**
   * Comma-separated document IDs that should never be processed by document
   * retention warnings or deletion.
   */
  @IsOptional()
  public DOCUMENT_RETENTION_EXCLUDED_DOCUMENT_IDS =
    this.toOptionalCommaList(
      environment.DOCUMENT_RETENTION_EXCLUDED_DOCUMENT_IDS
    ) ?? [];

  /**
   * Optional notification-service gRPC target for document retention warnings.
   */
  @IsOptional()
  public NOTIFICATION_GRPC_TARGET = this.toOptionalString(
    environment.NOTIFICATION_GRPC_TARGET
  );

  /**
   * Notification channel used for document retention warnings.
   */
  @IsOptional()
  public NOTIFICATION_GRPC_CHANNEL =
    this.toOptionalString(environment.NOTIFICATION_GRPC_CHANNEL) ?? "wecom";

  /**
   * CA certificate path used to verify the notification-service gRPC server.
   */
  @IsOptional()
  public NOTIFICATION_GRPC_CA_CERT_PATH = this.toOptionalString(
    environment.NOTIFICATION_GRPC_CA_CERT_PATH
  );

  /**
   * Client certificate path used for notification-service mTLS.
   */
  @IsOptional()
  public NOTIFICATION_GRPC_CLIENT_CERT_PATH = this.toOptionalString(
    environment.NOTIFICATION_GRPC_CLIENT_CERT_PATH
  );

  /**
   * Client private key path used for notification-service mTLS.
   */
  @IsOptional()
  public NOTIFICATION_GRPC_CLIENT_KEY_PATH = this.toOptionalString(
    environment.NOTIFICATION_GRPC_CLIENT_KEY_PATH
  );

  /**
   * TLS server name used when verifying the notification-service certificate.
   */
  @IsOptional()
  public NOTIFICATION_GRPC_SERVER_NAME =
    this.toOptionalString(environment.NOTIFICATION_GRPC_SERVER_NAME) ??
    "localhost";

  /**
   * Message deduplication window in notification-service.
   */
  @IsOptional()
  @IsNumber()
  @Min(0)
  public NOTIFICATION_GRPC_RATE_LIMIT_SECONDS =
    this.toOptionalNumber(environment.NOTIFICATION_GRPC_RATE_LIMIT_SECONDS) ??
    Day.seconds;
