...
const DocumentReportDialog = lazyWithRetry(
  () => import("~/components/DocumentReportDialog")
);
...
export const generateDocumentReport = createAction({
  name: ({ t }) => t("Generate report"),
  analyticsName: "Generate document report",
  section: ActiveDocumentSection,
  icon: <SparklesIcon />,
  visible: ({ activeDocumentId, stores }) => {
    if (!activeDocumentId) {
    ...
    return !!stores.policies.abilities(activeDocumentId).createReport;
  },
  perform: async ({ activeDocumentId, stores, t }) => {
    if (!activeDocumentId) {
      return;
