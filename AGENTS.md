...
## Planned Feature: Shared Document Report Agent

When adding report summaries for shared documents, prefer a hybrid architecture:

- Outline owns the product entry points, permissions, document access, report storage, audit logs, and UI.
- An external Agent service owns LLM calls, agent workflow orchestration, long-running jobs, retries, prompt versions, model configuration, and result validation.
- The Agent service must not directly decide document permissions or fetch arbitrary Outline data by document ID. It should only process content that Outline has already authorized and explicitly sent.

### Permission Model

- Check that the requesting user can read the source document before creating a report job.
- When report scope includes child or related documents, check read permission for each included document.
- Do not include any document content in the Agent request if the user cannot already access it in Outline.
- Treat report generation as a separate capability from document reading. Team members with read access may generate reports only when the team-level AI/report feature is enabled.
- Anonymous shared-link visitors should not be allowed to trigger report generation by default.
- Reports should inherit source document visibility unless a stricter product rule is implemented.
- Inserting a generated report into the document body should require document edit permission.
- Making a report visible to shared-link visitors should require an explicit product decision and permission check.
- Add audit records for report creation, included source documents, requesting user, status, Agent job ID, and completion time.

### Data Flow

1. The user starts report generation from the document UI.
2. The web app calls an Outline server RPC endpoint such as `reports.create`.
3. The server validates document access, report-generation capability, AI feature flags, and report scope.
4. The server creates a report job with `pending` status.
5. A worker builds a normalized report input from authorized document content only.
6. The worker sends the job to the external Agent service over an authenticated service-to-service API.
7. The Agent service generates a structured report and returns or callbacks with the result.
8. Outline validates the result shape, saves the report, updates job status, and notifies the client.

### Suggested Report Model

Use a dedicated report model instead of writing Agent output directly into document content.
...
- `reports.retry`
- `reports.delete`
- `reports.insertIntoDocument`

External Agent service endpoints may be HTTP-based initially:

- `POST /agent/report-jobs`
- `GET /agent/report-jobs/:id`
- `POST /agent/report-jobs/:id/cancel`

Use signed or token-authenticated service-to-service requests. Do not expose Agent credentials to the frontend.

### UX Expectations

- Add report generation to the document menu or a document-side panel.
- Let users choose report format and scope before creating a job.
- Show asynchronous job status instead of blocking the initial request.
- Save completed reports separately from the document body.
- Provide actions to view, copy, retry, delete, or insert the report into the document when the user has permission.
